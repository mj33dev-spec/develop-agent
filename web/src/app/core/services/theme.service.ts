...
export type StyleTheme = '플랫' | '글래스모피즘' | '뉴모피즘';
...
export class ThemeService {
  ...
  private styleThemeSubject = new BehaviorSubject<StyleTheme>('뉴모피즘');
  ...
  async initTheme() {
    try {
      ...
      this.setStyleTheme(settings.defaultTheme || '뉴모피즘');
    } catch (e) {
      ...
      if (saved) {
        try {
          ...
          this.setStyleTheme(parsed.defaultTheme || '뉴모피즘');
        } catch (err) {}
      }
    }
  }
  ...
  setStyleTheme(theme: string) {
    const validTheme: StyleTheme = (theme === '플랫' || theme === '글래스모피즘') ? theme : '뉴모피즘';
